# Edgewalker Growth Cohort 3 — Prospective Analysis Plan
...
- Market data: IEX.
- Production strategy: Full Roster Edgewalker with asymmetric bidirectional opening authority, automatic +1% Bank Day, and the production sizing selected at launch.
- Bearish opening doctrine: the confirmed Opening Impulse lane observes six completed bars inside the first 20 minutes, requires SOXL downside path and velocity, matching SOXS strength, continuing low pressure, and rejects both recovery and waterfall exhaustion before routing SOXS.
- Bullish opening doctrine: the Opening ORB lane builds the first 15-minute SOXL range, requires one completed confirmation bar, a 0.10% buffered break above the range high, SOXL at least 1% above its session open but no more than 9% extended, paired SOXS at least 0.5% below its open, and confirming bar direction before routing SOXL.
- Opening arbitration: at most one directional opening candidate may own the route. Ambiguous simultaneous qualification fails closed. Each opening lane gets one attempt per session.
- Bullish opening exit doctrine: dedicated 1% trailing stop, normal +1% Auto Bank eligibility, and a 9:55 ET time exit followed by handoff to the normal roster.
...
Cohort 2 was superseded after zero completed sessions when the bullish Opening ORB half was added to complete the early-session strategy. Its database records remain intact; no Cohort 2 observation is promoted into Cohort 3.
...
- Opening ORB and Opening Impulse trade count, realized P/L, MFE, MAE, exit reason, and downstream normal-roster trades.
...
All daily account returns use the broker-reported day P/L percentage captured by Edgewalker. Capital level is normalized in the primary return statistics while actual dollars, buying power, requested notional, effective notional, and entry-family attribution remain in the decision ledger for capacity, displacement, and slippage analysis.
...
No inference is made from a single best day, only traded days, or a backfilled subset. The retrospective IEX study used discovery, validation, and holdout partitions through 2026-08-18; those sessions are design evidence and are not prospective confirmation. Any post-launch rule change creates a new version and a new prospective cohort.
